...
void  initialisePIXELs(float *WrappedImageM, PIXEL *pixel, int image_width, int image_height)
{
	PIXEL *pixel_pointer = pixel;
	float *wrapped_image_pointer = WrappedImageM;
	int i, j;
...
void unwrap(const Mat & src, Mat & dst)
{
	...
	initialisePIXELs(WrappedImageM, pixel, image_width, image_height);

	calculate_reliability(WrappedImageM, pixel, image_width, image_height);
	...
	//float *UnwrappedImageM = (float *)dstImg.data;
	returnImage(pixel, UnwrappedImageM, image_width, image_height);
